...
begin
  ...
  intros ι U hU hUS,
  rw [set.image_subset_iff, set.preimage_Union] at hUS,
  obtain ⟨t, ht⟩ := hS (λ i, f⁻¹' (U i)) (λ i, hf.is_open_preimage (U i) (hU i)) hUS,
  use t,
  simp only [set.image_subset_iff, set.preimage_Union],
  exact ht,
end
