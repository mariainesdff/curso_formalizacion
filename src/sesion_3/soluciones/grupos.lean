...
begin
  split,
  { intro hxy,
    rw [← mul_one y, ← mul_inv_self x, ← mul_assoc y x x⁻¹],
    apply H.mul_mem hxy (H.inv_mem hx) },
  { intro hy,
...
{ carrier := { a : G | ∃ h ∈ H, a = g * h * g⁻¹ },
  ...
  mul_mem' := begin 
    rintro - - ⟨a, ha, rfl⟩ ⟨b, hb, rfl⟩, -- comprimimos el proceso de introducción de variables
    use (a * b),
    exact ⟨H.mul_mem ha hb, by group⟩, 
    -- la táctica `group` cierra metas que sólo utilizan los axiomas del grupo
  end,
}
...
begin
  ...
  split,
  { rintros ⟨c, hcH, hbc⟩,
    rw [hbc, h_ab a _, mul_assoc c a a⁻¹, mul_inv_self a, mul_one c],
    exact hcH },
  { intros hb,
    use b,
    refine ⟨hb, _⟩,
    { rw [h_ab a _, mul_assoc b _ _, mul_inv_self a, mul_one b], }}
end
...
{ carrier := {g : G | f g = 1 },
  one_mem' := begin exact map_one _, end,
  mul_mem' := begin 
    intros a b ha hb,
    simp only [set.mem_set_of_eq] at ha hb ⊢,
...
{ carrier := {h : H | ∃ g : G, g ∈ K ∧ f g = h },
  ...
  mul_mem' := begin
    rintros a b ⟨c, hcK, hac⟩ ⟨d, hdK, hbd⟩,
    use c * d,
    split,
    { exact mul_mem hcK hdK, },
    { rw [map_mul, hac, hbd] }
  end,
  inv_mem' := begin
    rintros a ⟨b, hbK, hab⟩,
